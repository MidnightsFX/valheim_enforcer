...
namespace ValheimEnforcer {
    internal class ValConfig {
        ...
        public static ConfigEntry<bool> NewCharacterClearKnownTexts;
        public static ConfigEntry<string> KnownTextPassthroughPrefixes;
        public static ConfigEntry<bool> EpicMMOKnownTextCompat;
        ...
        private void CreateConfigValues(ConfigFile Config) {
            ...
            KnownTextPassthroughPrefixes = BindServerConfig("Player Sync", "KnownTextPassthroughPrefixes", modules.compat.CompatKnownTexts.DefaultPrefixes, "Comma separated key prefixes in a character's known texts that are left to the mod that owns them instead of being tracked and enforced. Valheim's known texts are the compendium - the runestone and lore entries a character has read - but nothing namespaces the keys, so some mods keep per-player progression there. EpicMMO is the shipped case: its level, experience and attribute points are one entry each, all prefixed EpicMMOSystem. Progression is only captured on full saves, never on deltas, so enforcing such a key against the server's copy would roll back everything earned since the last one. A matched key is never stored in a server save and never overwrites the live player's own value. Matching is by prefix and case sensitive. Leave this empty to track known texts in full, and note that it does not exempt anything from NewCharacterClearKnownTexts.", advanced: true);
            // Parsed into a volatile snapshot read from worker threads (see CompatKnownTexts); keep it
            // current from the main thread, including on config reload and server sync.
            KnownTextPassthroughPrefixes.SettingChanged += (sender, args) => modules.compat.CompatKnownTexts.RefreshPrefixes();
            modules.compat.CompatKnownTexts.RefreshPrefixes();
            EpicMMOKnownTextCompat = BindServerConfig("Player Sync", "EpicMMOKnownTextCompat", true, "Only does anything when EpicMMO is installed, and only for a character joining this server for the first time under NewCharacterClearKnownTexts. EpicMMO does not read its level out of the known texts when something asks for it - it reads them once in a Game.SpawnPlayer patch and publishes what it found to the player's ZDO and custom data, which is where its own monster scaling reads a player's level from. That patch runs before the enforcer's join validation, so the reset left the character at level 1 while the world went on scaling monsters to the level they walked in with for the rest of the session. With this on, the reset is done ahead of every spawn patch when the server has already confirmed the character is new, so EpicMMO reads the reset state itself; when the join had to wait for that answer and there was nothing left to get in front of, EpicMMO is asked to read again afterwards instead. Turn it off to leave EpicMMO entirely alone, at the cost of the stale level above. Nothing here changes what is reset - only when, and who is told about it.", null, true);
            ...
            NewCharacterClearKnownTexts = BindServerConfig("Player Sync", "NewCharacterClearKnownTexts", true, "If enabled, a character joining this server for the first time forgets its known texts - the runestone and lore entries of the in-game compendium, read somewhere this server never saw. Vanilla's own reset does not cover these: Player.ResetCharacterKnownItems, which NewCharacterClearKnownRecipes runs, clears recipes, materials, crafting stations and trophies and leaves known texts untouched. That matters beyond the compendium, because some mods keep per-player progression in the same dictionary - EpicMMO stores a character's level, experience and attribute points there, one entry each - so with this off a first-time joiner whose items, skills, recipes and custom data were all reset still arrives at the level they reached elsewhere. KnownTextPassthroughPrefixes does not exempt anything from this; it governs what is enforced between joins, not what a character may bring in on its first one. Like the recipe and map resets this cannot be undone, so it only happens once the server has confirmed it holds no save for the character, and it is carried out by the client because known texts never reach the server. Never applies in singleplayer or to a listen host's own character. On by default.");
        ...
        internal static void PersistReceivedCharacterYaml(long sender, string yaml) {
            ...
            if (ValConfig.InternalStorageMode.Value) {
                ...
                try {
                    ...
                    modules.compat.CompatKnownTexts.StripPassthroughKeys(chara.Progress?.KnownTexts);
