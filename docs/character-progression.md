...
| **Full saves** | The server asks connected players for a complete character save every `FullSyncPullIntervalMinutes`, staggered `FullSyncMaxConcurrentPlayers` at a time so a large server does not spike its upload |
...
| `FullSyncMaxConcurrentPlayers` | Advanced | `5` | How many players are asked at once. Larger counts are staggered into successive waves to avoid a bandwidth spike. Lower it on a constrained upload or VPS host |
