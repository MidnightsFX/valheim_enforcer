...
A compressed file with the binaries is created in `<ValheimEnforcer>\Packages`ready for upload to ThunderStore.
...
A Unity project is included in this repository under `<ValheimEnforcer>\JotunnModUnity`.
...
4. Compile the project. This copies all assemblies into `<ValheimEnforcer>\JotunnModUnity\Assets\Assemblies`. Don't open Unity yet before this step, it will remove assembly references.
...
1. Copy all `assembly_*.dll` from `<ValheimDir>\valheim_Data\Managed` into `<ValheimEnforcer>\JotunnModUnity\Assets\Assemblies`. <br />
